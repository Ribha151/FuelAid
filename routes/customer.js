...
const baseCharges = {
	...
	"Engine Repair": 1000,
	// Fuel services
	"Petrol": 100, // Delivery base charge
	"Diesel": 100,
	"Fuel Tank Cleaning": 500,
	"Fuel Quality Testing": 600
  };
...
router.post('/customer/pay/:id', middleware.ensurecustomerLoggedIn, async (req, res) => {
  ...
  router.get('/customer/receipt/:id', middleware.ensurecustomerLoggedIn, async (req, res) => {
	try {
	  const request = await ServiceRequest.findById(req.params.id).populate('customer').populate('mechanic').populate('fueldeliveryboy');
...
doc.moveDown();// Title with underline and bold
	  ...
	  doc.fontSize(14).font('Helvetica').text(`Customer Name: ${request.customer.firstName} ${request.customer.lastName}`);
	  doc.fontSize(14).font('Helvetica').text(`Service Type: ${request.serviceCategory}`);
	  doc.text(`Sub Services: ${request.subServices.join(', ')}`);
	  if(request.mechanic){
		const mechanicName = request.mechanic ? `${request.mechanic.firstName} ${request.mechanic.lastName}` : "N/A";
		doc.text(`Mechanic Assigned: ${mechanicName}`);
	  }
	  if(request.fueldeliveryboy){
		const fuelName = request.fueldeliveryboy ? `${request.fueldeliveryboy.firstName} ${request.fueldeliveryboy.lastName}` : "N/A";
	  	doc.text(`Fuel Boy Assigned: ${fuelName}`);
	  }
