...
router.post("/auth/signup", middleware.ensureNotLoggedIn, async (req,res) => {
	...
	const { firstName, lastName, email, acn, dl, address, phone, password1, password2, role } = req.body;
	let errors = [];
  ...
  if (!geo) {
	...
	if (!firstName || !lastName || !email || !acn || !dl || !address || !phone ||!password1 || !password2) {
	...
	try
	{
		...
		if(user)
		{
			...
			return res.render("auth/signup", {
				title: "User Signup",
				firstName, lastName, errors, email, acn, dl, address, phone, password1, password2
			});
		}
		
		const newUser = new User({ firstName, lastName, email, acn, dl, address, latitude: geo.latitude,
			longitude: geo.longitude, phone, password:password1, role });
...
router.post("/auth/fueldel", async (req,res) => {
	...
	const { firstName, lastName, email, acn, dl, age, address, phone, experience, employer, vehicleType, vehicleNumber, role} = req.body;
	let errors = [];
	
	if (!firstName || !lastName || !email || !acn || !dl ||!age || !address || !phone || !experience || !employer || !vehicleType || !vehicleNumber || !role) {
		errors.push({ msg: "Please fill in all the fields" });
	}
	if(errors.length > 0) {
	...
	try
	{
		...
		if(hire)
		{
			...
			return res.render("auth/fueldel", {
				title: "Joining form",
				firstName, lastName, errors, email, acn, dl, age, address, phone, experience, employer, vehicleType, vehicleNumber, role
			});
		}
		
		const newHire = new Hire({ firstName, lastName, email, acn, dl, age, address, phone, experience, employer, vehicleType, vehicleNumber, role });
		await newHire.save();
